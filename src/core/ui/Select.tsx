...
const SelectItem = React.forwardRef<
  ...
  React.ComponentPropsWithoutRef<typeof SelectPrimitive.Item> & { showSelectedIcon?: boolean }
>(({ className, children, showSelectedIcon = true, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      `relative flex w-full cursor-default select-none items-center rounded-sm py-1.5 pl-${
        showSelectedIcon ? 8 : 2
      } pr-2 text-sm outline-none focus:bg-accent focus:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50`,
      className,
    )}
    {...props}
  >
    <If condition={showSelectedIcon}>
